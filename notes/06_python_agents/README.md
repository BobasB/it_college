...
1. Створіть проект з Sequential агентом:
   ```bash
...
3. Скопіюйте код з [`code_pipeline/agent.py`](code_pipeline/agent.py) та створіть pipeline для розробки коду.

4. :star: Запустіть Sequential агента:
   ```bash
   poetry run adk run code_pipeline
   ```

5. :star: Протестуйте з запитом: "Створи функцію для обчислення факторіалу числа". Агент послідовно виконає всі три етапи. Додайте результат у звіт.

6. :star: Поясніть у звіті переваги Sequential агента. Чому важливий порядок виконання підагентів?
...
1. Створіть проект з Loop агентом:
   ```bash
...
3. Скопіюйте код з [`story_improver/agent.py`](story_improver/agent.py) та створіть агента який покращує історію у циклі.

4. :star: Запустіть Loop агента:
   ```bash
   poetry run adk run story_improver
   ```

5. :star: При запуску введіть тему: "робот який навчився мріяти". Агент буде ітеративно покращувати історію. Додайте результат у звіт.

6. :star: Поясніть у звіті як працює механізм завершення циклу через `exit_loop` функцію.
...
3. Скопіюйте код з [`research_team/agent.py`](research_team/agent.py). Цей агент виконує три дослідження паралельно та обʼєднує результати.

4. :star: Запустіть Parallel агента:
   ```bash
   poetry run adk run research_team
   ```

5. :star: Запитайте: "Які останні тренди у технологіях?". Агент виконає три дослідження паралельно та об'єднає результати. Додайте звіт у ваш звіт.

6. :star: Поясніть у звіті переваги паралельного виконання. Чому це швидше ніж послідовне виконання трьох досліджень?
